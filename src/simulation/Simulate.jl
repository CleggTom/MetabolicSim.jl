...
"""
function simulate(x0::Vector{Float64}, p::Parameters; t_start=0.0, t_end=1.0, tsave = [])
    prob = OrdinaryDiffEq.ODEProblem(dx!,x0,(t_start,t_end),deepcopy(p))
    sol = DiffEqBase.solve(prob, OrdinaryDiffEq.AutoTsit5(OrdinaryDiffEq.Rosenbrock23()), saveat = tsave)
    return(sol)
end
...
"""
function simulate(x0::Vector{Float64}, p::Parameters, assembly_rules::Any; t_start=0.0, t_end=1.0, tsave = [])
    #for equilibrium assembly
    if assembly_rules[1] == :equilibrium
        prob = OrdinaryDiffEq.ODEProblem(dx!,x0,(t_start,t_end),deepcopy(p))
        sol = DiffEqBase.solve(prob, OrdinaryDiffEq.AutoTsit5(OrdinaryDiffEq.Rosenbrock23()), callback = assembly_rules[2], saveat = tsave)
    ...
    elseif assembly_rules[1] == :at_time_stop
        ...
        sol = DiffEqBase.solve(prob, OrdinaryDiffEq.AutoTsit5(OrdinaryDiffEq.Rosenbrock23()), callback = assembly_rules[2], tstops = assembly_rules[3], saveat = tsave)
    else
